// З новим роком)  esse quam videri
...
function t4() {
  let fewSpans = document.querySelectorAll(".task-4");
  fewSpans.forEach((e) => {
    a4_res.push(e.attributes.data.value);
    //a4_res.push(e.getAttribute("data"));
  });
  //console.log(fewSpans)
  //console.log(a4_res)
...
function t6() {
  a6_res = str6.split("");
  console.log(a6_res);
  document.querySelector(".out-6").innerHTML = a6_res.join(" ");
}
...
function t7() {
  a7_res = str7.split(" ");
  console.log(a7_res);
  document.querySelector(".out-7").innerHTML = a7_res.join(" ");
}
...
function t10() {
  for (let key in a10) {
    ...
    a10_res += encodeURIComponent(key) + "=" + encodeURIComponent(a10[key]); // encode/decode... кодування/розкодування різних символів які мжуть бути неправильно передані(думаю більше, щоб не були "неправильно інтерпритовані")
  }
  // console.log(a10_res);
