...
function t2() {
  ...
  // console.log(output);
  document.querySelector(".out-2").innerHTML = output;
}
...
function t7() {
  ...
  document.querySelector(".out-7").innerHTML = output;
}
...
function t9() {
  let f = +document.querySelector(".i-91").value; // f = first input
  let s = +document.querySelector(".i-92").value; // s = second input
  let result = "";
  let i;
  let n;
...
function t10() {
  let min = 1950;
  let max = 1970;
  let i = min; // щоб було зручніше для сприйняття в циклі.
  let result = "";
...
function t15() {
  ...
  for (let i = 0; i <= 10; i++) {
    result += 10 - i + "_" + i + "_"; // це не підказка, це готовий розв'язок. Аnyway, дякую.
  }
