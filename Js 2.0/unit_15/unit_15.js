...
const f4 = () => {
  ...
  let haveOrNotValue = false;

  if (s4.has(input)) {
    haveOrNotValue = true;
  }

  document.querySelector(".out-4").innerHTML = haveOrNotValue;
};
...
const f6 = () => {
  let uniqueItems = new Set(a6);
  document.querySelector(".out-6").innerHTML = uniqueItems.size;
...
let s8 = new Set([
  ...
  4, 3, 2, 335, 2, 23, 41, 3, 4, 1, 1, 4, 2, 2, 4, 5, 24, 5, 3, 22, 56, 6,
]);
let ar8 = [];
...
const f9 = (our_set) => {
  let toString = Array.from(our_set).join(" ");
  return toString;
...
const f10 = (out_set, elem) => {
  let toString = Array.from(out_set).join(" ");
  document.querySelector(elem).innerHTML = toString;
};
