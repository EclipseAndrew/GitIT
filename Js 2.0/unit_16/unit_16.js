...
function f3() {
  ...
  for (let i = 0; i < out.length; i++) {
    //можна іще через for...in, але там є свої нюанси.
    out[i].innerHTML = 3;
  }

  console.log(out); //HTMLCollection
}
...
function f4() {
  ...
  for (let i = 0; i < out.length; i++) {
    out[i].innerHTML += 4; // тепер дописуємо в кожен, да? (я міг не зрозуміти умову)
                          // надіюсь правильно, не хочу помилитись знову.
  }
...
function f7() {
  ...
  for (let i = 0; i < a7.length; i++) {
    newObj[a7[i].id] = a7[i].name; // !просто
  }
  //console.log(newObj)
  return newObj;
}
...
function f9() {
  ...
  for (let i = 0; i < a9.length; i++) {
    if (a9[i].length > lengthOfArr) {
      lengthOfArr = a9[i].length - 1;
    }
  }
...
function f11() {
  ...
  document.querySelector(".out-11").innerHTML = moreThan10;
}
...
function f12() {
  ...
  document.querySelector(".out-12").innerHTML = iterate.join(" ");
}
...
function f13() {
  ...
  for (let each of a13) {
    iterateOfString += each + " ";
  }
  document.querySelector(".out-13").innerHTML = iterateOfString;
}
...
function f14() {
  ...
  for (let each of a14) {
    convertToString += each + " ";
  }
  document.querySelector(".out-14").innerHTML = convertToString;
}
...
function f15() {
  let rewriting = document.querySelectorAll(".out-15");

  for (let i of rewriting) {
    i.innerHTML = 15;
  }
