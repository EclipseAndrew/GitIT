...
function f2() {
  let rowLength = +document.querySelector(".i-2").value;
  console.log(rowLength);

  const xhr = new XMLHttpRequest();
  xhr.open("GET", URL + `/api/25/random/random-string?length=${rowLength}`);
  xhr.setRequestHeader("APIKEY", APIKEY);
  xhr.onload = () => {
    const data = JSON.parse(xhr.response);
...
function f3() {
  ...
  xhr.onload = () => {
    const data = JSON.parse(xhr.response);
    let outString = data.result.map((item) => {
      return item.race;
    });
    document.querySelector(".out-3").innerHTML = outString.join(" ");
  };
  xhr.send();
}
...
function f7() {
  const outPictures = document.querySelector(".out-7");
  outPictures.innerHTML = "";
...
function f8() {
  ...
  xhr.onload = () => {
    const data = JSON.parse(xhr.response);
    console.log(data);
    document.querySelector(".out-8").innerHTML = data.result.length;
  };
  xhr.send();
}
...
function f9() {
  let numOfEmployee = document.querySelector(".i-9").value;
  console.log(numOfEmployee);

  const xhr = new XMLHttpRequest();
  xhr.open("POST", URL + `/api/25/employee/read/${numOfEmployee}`);
  xhr.setRequestHeader("APIKEY", APIKEY);
  xhr.onload = () => {
    const data = JSON.parse(xhr.response);
    console.log(data);
...
function f10() {
  let numOfEmployee = document.querySelector(".i-10").value;

  const xhr = new XMLHttpRequest();
  xhr.open("GET", URL + `/api/25/employee/read/${numOfEmployee}`);
  xhr.setRequestHeader("APIKEY", APIKEY);
  xhr.onload = () => {
    const data = JSON.parse(xhr.response);
    console.log(data);
...
function f11() {
  let lengthOfPassword = document.querySelector(".i-11").value;
  ...
  param.append("length", lengthOfPassword);
  xhr.onload = () => {
    console.log(xhr.status);
    const data = JSON.parse(xhr.response);
    console.log(data);
...
function f12() {
  let lengthOfPassword = document.querySelector(".i-12").value;
...
function f13() {
  let lengthOfPassword = document.querySelector(".i-13").value;
  let symbols = document.querySelector("#ch-131").checked ? 1 : 0;
  let uppercase = document.querySelector("#ch-132").checked ? 1 : 0;
  ...
  param.append("length", lengthOfPassword);
  param.append("symbols", symbols);
  param.append("uppercase", uppercase);
  xhr.onload = () => {
    console.log(xhr.status);
    const data = JSON.parse(xhr.response);
    console.log(data);
...
function f15() {
  const outPictures = document.querySelector(".out-15");
  outPictures.innerHTML = "";
  ...
  xhr.onload = () => {
    const data = JSON.parse(xhr.response);
    console.log(data);
    const arrOfImg = data.result.map((e) => {
      return e.image;
    });

    for (let item of arrOfImg) {
      const img = document.createElement("img");
      img.src = URL + item;
      outPictures.append(img);
    }
  };
  xhr.send();
}
...
function f16() {
  ...
  xhr.onload = () => {
    const data = JSON.parse(xhr.response);
    console.log(data);
    const worldsList = data.worlds.map((elem) => elem.title);
    console.log(worldsList);
    document.querySelector(".out-16").innerHTML = worldsList.join(" ");
  };
  xhr.send();
}
...
function f17() {
  let nameWorld = document.querySelector(".s-17").value;
  console.log(nameWorld);

  const xhr = new XMLHttpRequest();
  xhr.open("GET", URL + `/api/25/gow/world/${nameWorld}`);
  xhr.setRequestHeader("apikey", APIKEY);
  xhr.onload = () => {
    const data = JSON.parse(xhr.response);
    console.log(data);
...
function f18() {
  let outPictures = document.querySelector(".out-18");
  outPictures.innerHTML = "";
