...
function f2() {
  ...
  const xhr = new XMLHttpRequest();
  xhr.open("GET", `${URL}/api/25/random/random-string?length=${value}`);
  xhr.setRequestHeader("apikey", `${APIKEY}`);
  xhr.onload = () => {
    console.log(xhr.status);
    const data = JSON.parse(xhr.response);
...
function f3() {
  ...
  xhr.onload = () => {
    ...
    data["result"].forEach(
      (item) =>
        (document.querySelector(".out-3").innerHTML += `${item["race"]} `)
    );
  };
  xhr.send();
}
...
function f7() {
  ...
  xhr.onload = () => {
    ...
    const imageSrc = data["result"]["image"];
    
    const img = document.createElement("img");
    img.setAttribute("src", `${URL}${imageSrc}`);
...
function f9() {
  ...
  const xhr = new XMLHttpRequest();
  xhr.open("POST", `${URL}/api/25/employee/read/${value}`);
  xhr.setRequestHeader("apikey", `${APIKEY}`);
  xhr.onload = () => {
    console.log(xhr.status);
    const data = JSON.parse(xhr.response);
...
function f10() {
  const value = +document.querySelector(".i-10").value;
  const xhr = new XMLHttpRequest();
  xhr.open("GET", `${URL}/api/25/employee/read/${value}`);
  xhr.setRequestHeader("apikey", `${APIKEY}`);
  xhr.onload = () => {
    console.log(xhr.status);
    const data = JSON.parse(xhr.response);
...
function f11() {
  const value = +document.querySelector(".i-11").value;
  const xhr = new XMLHttpRequest();
...
function f12() {
  ...
  const symbols = document.querySelector("#ch-12").checked ? 1 : 0;
  console.log(symbols);
  const xhr = new XMLHttpRequest();
  xhr.open("POST", `${URL}/api/25/random/generate-password`);
  xhr.setRequestHeader("apikey", `${APIKEY}`);
...
function f14() {
  const race = document.querySelector(".s-14").value;
  const xhr = new XMLHttpRequest();
  ...
  xhr.onload = () => {
    ...
    document.querySelector(
      ".out-14"
    ).innerHTML = `${data["result"]["description"]}`;
  };
  xhr.send();
}
...
function f17() {
  const value = document.querySelector(".s-17").value;
  const xhr = new XMLHttpRequest();
  xhr.open("GET", `${URL}/api/25/gow/world/${value}`);
  xhr.setRequestHeader("apikey", `${APIKEY}`);
  xhr.onload = () => {
    console.log(xhr.status);
    const data = JSON.parse(xhr.response);
...
function f18() {
  ...
  xhr.onload = () => {
    ...
    for (let key in data["rune"]) {
      console.log(`${URL}${data["rune"][key]}`);
      const image = document.createElement("img");
      image.setAttribute("alt", `${key}`);
      image.setAttribute("src", `${URL}${data["rune"][key]}`);
      out.append(image);
    }
  };
  xhr.send();
