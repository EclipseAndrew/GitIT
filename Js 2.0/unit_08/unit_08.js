...
function t6() {
  let result = "";
  let howManyStirngs = document.querySelector(".i-6").value;
  let count = 0;

  while (count < howManyStirngs) {
    let secCount = 0;
    while (secCount < 6) {
      result += "*";
      secCount++;
...
function t7() {
  let result = "";
  let count = +document.querySelector(".i-7").value; // "+" вказую явно тип number (просто так)
...
function t8() {
  let result = "";
  let count = +document.querySelector(".i-81").value;
  let secCount = +document.querySelector(".i-82").value;
...
function t9() {
  let result = "";
  let count = +document.querySelector(".i-91").value;
  let secCount = +document.querySelector(".i-92").value;
...
function t12() {
  ...
  // призначаєм property (style.background) під час виконання коду (неявно). Можна відслікувати в console коли викликати NodeList або HTMLCollection. Вроді так.
  console.log(divs[0].style.background); //на прикладі першого div в списку.
}
...
function t13() {
  let getInput = document.querySelectorAll(".i-13");
  let i = 0;

  while (i < getInput.length) {
    getInput[i].value = i + 1;
    i++;
  }
  // Виправлений варіант на підставі обговорень даної таски в чаті.
}
...
function t14() {
  ...
  console.log(value);
  document.querySelector(".out-14").innerHTML = value;
}
