...
function f1() {
  ...
  document.querySelector(".out-1").innerHTML = output;
}
...
function f3() {
  let firstInt = +document.querySelector(".i-31").value;
  let secondInt = +document.querySelector(".i-32").value;
  let result;
...
function f4() {
  ...
  let mustBeLessThanCurrent = input + 18; //назна довга, не подобається (не зміг придумати кращy)
  let currentDate = new Date().getFullYear(); //https://www.w3schools.com/jsref/jsref_getfullyear.asp
  let result;

  if (mustBeLessThanCurrent <= currentDate) {
    result = 1;
  } else {
    result = 0;
  }
  ...
  // console.log(mustBeLessThanCurrent);
}
...
function f7() {
  let inputInt = +document.querySelector(".i-71").value;
  let power = +document.querySelector(".i-72").value;
  let exponentiate = inputInt ** power;

  document.querySelector(".out-7").innerHTML = exponentiate;
}
...
function f9() {
  ...
  document.querySelector(".out-9").innerHTML = output;
}
...
function f10() {
  let i = document.querySelector(".s-100").value;
...
function f13() {
  document.querySelector(".out-13").innerHTML =
    typeof (/*+*/ document.querySelector(".i-130").value); // умова задачі(string)... буду уважніший.

  // Ви згадували цей приклад у відео...  дозволяється введення літери "е" що означає піднести до степеня. Знаю що це спосіб зекономити запис. Наприклад для обмеженного місця на дисплеї старих калькуляторів. Тай видно перекочувало.
}
