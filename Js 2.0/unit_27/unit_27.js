...
function resolveF1(data) {
  console.log(data);
  const out = data["random-number"] > 50 ? "true" : "false";
...
function rejectError(err) {
  console.log(err);
  document.querySelector(".out-2").innerHTML = err.status;
}
...
function f4() {
  ...
  fetch(URL + "/api/27/employee/random-email", {
    ...
    .then((response) => response.json())
    .then(
      (data) => (document.querySelector(".out-4").innerHTML = data.result.name)
    );
}
...
function f5() {
  ...
  fetch(URL + "/api/27/gow/random-world", {
    ...
    .then((data) => {
      console.log(data);
      return (document.querySelector(".out-5").innerHTML = data.world.governor);
    });
}
...
function f6() {
  ...
  Promise.all([promise_1, promise_2]).then((data) => {
    ...
    // console.log(data[1]['random-number'])
    const totalSum = data[0]["random-number"] + data[1]["random-number"];
    document.querySelector(".out-6").innerHTML = totalSum;
  });
}
...
function f8() {
  out8.innerHTML = "";
  const firstRace = document.querySelector(".s-81").value;
  const secondRace = document.querySelector(".s-82").value;
  //  console.log(firstRace,secondRace)
  const reqHead = new Headers();
  reqHead.append("apikey", APIKEY);

  const pr1 = new Promise((resolve) => {
    fetch(URL + "/api/27/sr/read/" + firstRace, {
      headers: reqHead,
    }).then((data) => resolve(data.json()));
  });

  const pr2 = new Promise((resolve) => {
    fetch(URL + "/api/27/sr/read/" + secondRace, {
      headers: reqHead,
    }).then((data) => resolve(data.json()));
  });
  ...
  Promise.all([pr1, pr2]).then((data) => {
    ...
    firstImg.src = URL + data[0].result.image;

    const secondImg = document.createElement("img");
    secondImg.src = URL + data[1].result.image;

    document.querySelector(".out-8").append(firstImg);
    document.querySelector(".out-8").append(secondImg);
  });
}

document.querySelector(".b-8").onclick = f8;

// Думав не потягну...  потягнув)
// Справді все доволі логічно, просто потрібно бути уважним і приділяти достатньо уваги (в кожного по свойму)
// Дякую за цей курс)) Було дуже цікаво, і я з нетерпінням очікую розширення цього та інших курсів (що нам цих 400+ завдань(сказав той хто ще не зробив 14 sprint)).
// І ще раз дякую, Ви дуже класний спеціаіст, наставник, ютубер і дуже приємна людина(Хоч ми і не знайомі).
