...
function f6() {
  ...
  let value = document.querySelector(".i-61").value;
  a6[key] = value;
  console.log(a6);
}
...
function f11() {
  ...
  for (let key in a11) {
    if ([key] == input) {
      delete a11[key]; /* або input*/
      f5(a11, ".out-11");
    }
  }

  // if (a11.hasOwnProperty(input)) {
  //   delete a11[input];
  //   f5(a11, ".out-11");
  // }
}
...
function f12() {
  ...
  for (let key in a12) {
    if (a12[key] == input) {
      delete a12[key];
      // f5(a12, ".out-12"); //  сам собі придумав що має видаляти перший зустрічний елемент.
      // break;
    }
  }
...
function f20() {
  ...
  for (let key in a20) {
    for (let i = 0; i < a20[key].length; i++) {
      if (a20[key][i][1] == 2) {
        out += a20[key][i][0] + " "; //тяжко але зробив ( це не складно але потрібно більше практики )
      }
    }
  }
