...
function t2() {
  a2_res = a2.map((element) => element ** 2);
  return a2_res;
}
...
function t5() {
  a5_res = a5.filter((int) => {
    return typeof int === "number" && int % 2 === 0;
  });
  return a5_res;
}
...
function t6() {
  a6_res = a6.filter((int) => {
    return typeof int === "number" && int > 14;
  });
  return a6_res;
}
...
function t7() {
  a7_res = a7.map((each) => each.toLowerCase());
  return a7_res;
}
...
// Task 11 ============================================
/*  Дан массив a11 = [2, 13, 14, -7, 9, 5, 0и, -2, 14]. С помощью filter, переберите массив a11 и создайте новый массив a11_res куда добавьте только те элементы массива ИНДЕКС которых - четный. Возвратите a11_res.*/

let a11 = [2, 13, 14, -7, 9, 5, 0, -2, 14];

function t11() {
  let a11_res = a11.filter((e, i) => {
    return i % 2 === 0;
  }); // було ...filter((e, i) => i % 2 === 0);  це в цьому проблема?(та вроді не повинна)
  return a11_res;
}
...
function t14() {
  return (
    a14.includes(a14_sym.toLowerCase()) || a14.includes(a14_sym.toUpperCase())
  );
}
...
function t15() {
  let a15_res = a15.filter((e) => {
    return e.name !== "" && e.pnum.length === 6;
  });
  return a15_res;
}
...
// це була проблема через відсутність return (в функції)??
