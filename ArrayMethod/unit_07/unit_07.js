...
const f10 = () => {
  let res_neg = a10.reduce((accum, item) => {
    if (item < 0) return accum + item;
    return accum;
  });

  let res_posit = a10.reduce((accum, item) => {
    if (item > 0) return accum + item;
    return accum;
  }, 0);

  let res = [res_posit, res_neg];
...
const f12 = () => {
  let res = a12.reduce((accum, item, index) => {
    if (item > a12[accum]) {
      return index;
    } // в умові звернення за індексом (я поняв)
    return accum;
  }, 0);
...
const f15 = () => { 
  let res = a15.reduce((accum, item, index, array) => {
    accum.sum += item;
    if (index === array.length - 1) {
      accum.middleValue = accum.sum / array.length;
    }
    return accum;
  }, { sum: 0, middleValue: 0 });
  document.querySelector(".out-15").innerHTML = res.middleValue.toFixed(0); // надіюсь проблема була не в toFixed()
};
...
const f16 = () => {
  ...
  console.log(out); //  я так поняв що це "движок" сам визначає порядок заповнення об'єкта в моєму випадку {17: 'Bristol', 45: 'Ivar', 464: 'Astor'}
  // хз мабуть це сортування за ключами (від меншого до більшого значення)

  let rokirovka = out.slice(0, 15); // це як той прікол про "справжні чоловіки div центрують не через margin: 0 auto; а через &nbsp".
...
const f17 = () => {
  ...
  a17_res = Object.keys(a17).reduce((acc, key) => {
    acc.push(a17[key]);
    return acc;
  }, []);
...
const f18 = () => {
  let res = a18.reduce((accum, item, index) => {
    return item !== undefined ? accum + index + " " + item + "<br>" : accum;
  }, "");
...
const f20 = () => {
  ...
  let res = a20.reduce((accum, item) => {
    ...
    accum.unshift(item) // accum.push(item).revers()
    return accum
