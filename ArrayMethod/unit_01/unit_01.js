...
const f1 = () => {
 ...
 document.querySelector(".out-1").innerHTML = a1.indexOf(input);
...
const f2 = () => {
  ...
  const finder = a2.indexOf(input);
  const res = finder != -1 ? finder : "false";
  out.innerHTML = res;
};
...
const f3 = () => {
  ...
  const finder = a3.indexOf(input);
  const res = finder != -1 ? "true" : "false";
  out.innerHTML = res;
};
...
const f4 = (arr, elem) => {
  ...
  // бо 2 та "2" it's not the same
  //і тут ми локально маніпулюємо string та numb які чітко прописані в array а не як у випадку отримування значення через input що автоматично приведе значення до типу string
};
...
const f5 = () => {
  const elem = +document.querySelector(".i-5-1").value;
  const index = +document.querySelector(".i-5-2").value;
  const out = document.querySelector(".out-5");

  if (isNaN(elem) && isNaN(index)) {
    return (out.innerHTML = "введіть цифри");
  }

  const finderValue = a5.indexOf(elem, index);
  out.innerHTML = finderValue;
...
const f6 = () => {
  //все буде як string. Можна зробити строго щось одне або number або string. Або для тих хто знає що, і чому він вводе, можна створити checkbox і повішати на нього тип number.
  const input = document.querySelector(".i-6").value;
  document.querySelector(".out-6").innerHTML = a6.indexOf(input);
};
...
const f8 = () => {
  ...
  let nextFindersIndex = 0;
  let found = false;

  const recursiveSearch = () => {
    let currentIndex = a8.indexOf(input, nextFindersIndex);
    if (currentIndex !== -1) {
      res08.push(currentIndex);
      nextFindersIndex = currentIndex + 1;
      found = true;
      recursiveSearch();
    } 
  };
  ...
  document.querySelector(".out-8").innerHTML = res08.join(", ");
};
...
const f10 = (arr, elem) => {
  const numType = arr.indexOf(elem);
  const stringType = arr.indexOf(elem + "");
  return numType < stringType ? numType : stringType;
};
