...
const f4 = (arr, delimiter) => {
  ...
  // //     cutLastElem += toStr[i]
  // //   }
  // //   console.log(cutLastElem)

  //   toStr = toStr.substring(0,toStr.length-1)
  //   console.log(toStr)

  //   //  return cutLastElem;
  //    return toStr  // чого коли вже написав return приходе в голову вписати if в  цикл...  холера!

  let toString = "";
  for (let i = 0; i < arr.length; i++) {
    if (i !== arr.length - 1) {
      toString += arr[i] + delimiter;
    } else {
      toString += arr[i];
    }
  }
...
const f10 = () => {
  let a10 = [];
  a10_res = [...s10].filter((e) => {
    e > "l" ? a10.push(e) : "";
  }); // можна не приводити до charCodeAt() бо і так фільтрує за unicode
  console.log(a10);
  document.querySelector(".out-10").innerHTML = a10.join(" ");
};
