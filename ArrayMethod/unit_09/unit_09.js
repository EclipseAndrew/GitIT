...
let a2 = [13, 15, 22, 23, 35, 72];
let a2_from = 23;
let a2_to = 27;

const f2 = () => {
  const res = a2.find((e) => e > a2_from && e < a2_to);
...
const f4 = () => {
  const strArr = str4.split("");
  const res = strArr.find((e) => e > a4_from && e < a4_to);
...
const f5 = () => {
  const res = a5.findIndex((e) => e > a5_from && e < a5_to);
...
const f6 = () => {
  const copyA6 = [...a6]; // бо reverse мутує оригінал.
  const revers = copyA6.reverse();  // знайшов  toReversed() він не мутує оригінал 
  const res = revers.findIndex((e) => {
    if (e < a6_to) {
      return true;
    }
  });
  const finalRes = a6.length - 1 - res; // (a6.length - 1) - res 
document.querySelector('.out-6').innerHTML = finalRes 
};
...
const f7 = () => {
    const res = str7.lastIndexOf(a7_1) // ...я завтикав
...
const f8 = () => {
    const res = str8.toLowerCase().lastIndexOf(a8_1)
...
const f10 = () => {
    a10_res = []
    //  a11.forEach(index => { 
    //  if (a10[index] !== undefined){ return a10_res.push(a10[index])}
    // })
    a10_res = a10.filter((e, i) => a11.includes(i))
