...
const f2 = () => {
  ...
  document.querySelector(".out-2").innerHTML = a2.indexOf(input) !== -1 ? a2.indexOf(input) : 'false';
};
...
const f4 = () => {
  ...
  valueOfKey === "" ? (out.innerHTML = "false") : (out.innerHTML = valueOfKey);
};
...
const f5 = () => {
  const i_item = +document.querySelector(".i-5-1").value; //i_item = input_item
  const i_index = +document.querySelector(".i-5-2").value;
  const out = a5.includes(i_item, i_index);
...
const f6 = () => {
  const input = document.querySelector(".i-6").value; // при порожньому input все буде true бо буде includes("")
  let result = false;
  if (input !== "") {
    for (let elem of a6) {
      if (elem.includes(input)) {
        //то понятно чутливий до регістру (а якщо в умові не чутливий до регістру тоді просто привести до нижнього чи верхнього)
        result = true;
        break;
      }
    }
...
const f9 = () => {
  ...
  let out = 0;
  const lowerCase = a9.map((e) =>
    typeof e === "string" ? e.toLowerCase() : e
  ); 

  !isNaN(input) ? (out = a9.includes(+input)) : (out = a9.includes(input));
  out === false ? (out = lowerCase.includes(input)) : "";
...
const f11 = () => {
  let c = [1, 2]; //false      бо includes працює за порівнянням змісту а в масивів та об'єктів зберігаються посилання і тому вони будуть різними.
  c = {a:1}; // false
  c = true;// true
  c = ''; // true
  c = [1];// false
