...
function t1(elem) {
  ...
  function r1(element) {
    // console.dir(element)
    out += element.tagName + " ";
    Array.from(element.children).forEach((child) => r1(child));
...
function t2(elem) {
  ...
  function r2(element) {
    Array.from(element.innerText).map((e) => {
      console.log(+e); //кров з очей, але працює.
      if (!isNaN(+e)) s += +e;
    });
  }
  console.log(s);
...
function t3(elem) {
  ...
  function r3(element) { 
    ...
    Array.from(nodes).forEach((e) => {// мене розриває в середині https://youtu.be/MVbNHCstkqY?t=11
      if (e.nodeName === "#text") { 
        out += e.textContent;
      }
      r3(e);
