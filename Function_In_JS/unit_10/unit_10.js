...
document.querySelector(".b-2").addEventListener("click", () => {
  document.querySelector(".out-2").textContent = t2(6);
});
...
function t8() {
  const num = randomInteger(1000, 9000) + "";
  console.log(num);
  return +num[0] + +num[1] === +num[2] + +num[3] ? num : t8();
}
...
function t10(key, obj) {
    if (typeof obj === "object") {
      if (key !== "parent" && !(obj?.age)) { //  !("age" in obj))   
        ar10_res.push(key);
      }
      for (let subKey in obj) { 
        t10(subKey, obj[subKey]); 
    }
  }
}
