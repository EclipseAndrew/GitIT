...
function getRandomInt() {
  return (out1.innerHTML = Math.floor(117 + Math.random() * (132 + 1 - 117)));
}
window.addEventListener("load", () => getRandomInt());
...
const randInt = (minimum, maximum) => {
  return (out2.innerHTML = Math.floor(
    minimum + Math.random() * (maximum + 1 - minimum)
  ));
};
window.addEventListener("load", () => {
  const randomNumber = randInt(min, max);
  out2.innerHTML = randomNumber;
});
...
function getRandomInt2(minimum, maximum) {
  const randInt = Math.floor(minimum + Math.random() * (maximum + 1 - minimum));
  randInt % 2 === 0 ? even() : odd();
  console.log(randInt);
